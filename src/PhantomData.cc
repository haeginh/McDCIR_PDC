...
PhantomData::PhantomData(G4String _filePrefix)
{
	...
	G4String filePrefix = string(getenv("DCIR_PHANTOM_DIR"))+ _filePrefix;
...
void PhantomData::OrganDoseRead(G4String doseFile){
	...
	if(!ifs.is_open()) {
		...
		G4Exception("PhantomData::OrganDoseRead","",FatalErrorInArgument,
				G4String("      There is no " + doseFile ).c_str());
	}

	organ2dose = Eigen::MatrixXd::Zero(100, materialMap.rbegin()->first+1);
	G4String aLine;
	G4int doseMaxID(0);
	while(!ifs.eof()){
		getline(ifs, aLine);
		...
		doseMaxID = doseID>doseMaxID? doseID:doseMaxID;
		G4String name; ss>>name; doseNameMap[doseID] = name;
		G4int organID;
		while(ss>>organID){
			organ2dose(doseID, organID) = 1.;
			// if(organ2dose.find(organID)==organ2dose.end()) organ2dose[organID] = {doseID};
			// else	                                       organ2dose[organID].push_back(doseID);
		}
	}
	ifs.close();
	doseNameMap[0] = "RBM(DRF)";
	doseNameMap[1] = "BF(DRF)";
	organ2dose.conservativeResize(doseMaxID+1, organ2dose.cols());
}

void PhantomData::EffectiveDoseRead(G4String effFile)
{
	std::ifstream ifs(effFile);
	if(!ifs.is_open()) {
		// exception for the case when there is no *.material file
		G4Exception("PhantomData::EffectiveDoseRead","",FatalErrorInArgument,
				G4String("      There is no " + effFile ).c_str());
	}
	effCal = RowVectorXd::Zero(organ2dose.rows());
	// effectiveDoseInfo.clear();
	G4int id;
	G4double factor;
	while(ifs>>id>>factor)
		effCal(id) = factor;
		// effectiveDoseInfo[id] = factor;
	ifs.close();	
}
...
void PhantomData::MaterialRead(G4String materialFile)
{
	...
	for(auto iter:materialIndexMap){
		...
		G4Material* mat = new G4Material(organNameMap[idx], densityMap[idx], G4int(iter.second.size()), kStateSolid, NTP_Temperature, STP_Pressure);
