...
:worldLogical(0) ,worldPhysical(0), table_default_trans(1.12*m, -0.3*m, 0.7*m), carm_isocenter(1120 * mm, 600 * mm, 1135 * mm),
focalLength(810*mm)
{
...
void DetectorConstruction::ConstructOperatingTable()
{
	...
	frame_rotation_matrix = new G4RotationMatrix();
 	pv_frame = 	new G4PVPlacement(frame_rotation_matrix,  table_default_trans + frame_ralative_to_table, lv_frame, "pv_frame", worldLogical, false, 100);

	// phantom box
	new G4PVPlacement(0, G4ThreeVector(0,frame->GetYHalfLength()-((G4Box*) lv_phantomBox->GetSolid())->GetYHalfLength()-10*cm,halfZ-((G4Box*) lv_phantomBox->GetSolid())->GetZHalfLength()),lv_phantomBox, "phantom box", lv_frame, false, 200);
...
G4LogicalVolume* DetectorConstruction::ConstructPatient()
{
	...
	for(int i=0;i<numT;i++)
	{
		...
		new G4PVPlacement(0, G4ThreeVector(), lv_tet, "tet", lv_phantomBox, false, 300);
