...
G4bool BoneScorer::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
    ...
    auto idx = phantom->GetIdx(copyNo);

	G4double rbmR, bsR;
	if(!phantom->GetBoneMassRatio(idx.first, idx.second, rbmR, bsR)) return FALSE;
	...
	G4double CellFlux = stepLength / phantom->GetVol(idx.first, idx.second);
	...
	tie(rbmDrfDose, bsDrfDose) = GetDRFdose(idx.first, energy, CellFlux, idx.second);
	...
	EvtMap->add(idx.first*10 + 0, rbmDrfDose);
	EvtMap->add(idx.first*10 + 1, bsDrfDose);
...
std::pair<G4double, G4double> BoneScorer::GetDRFdose(G4int phantomID, G4double energy, G4double cellFlux, G4int organID)
{
	G4double rbmDRF, bsDRF;
	phantom->GetData(phantomID)->GetDRF(energy, organID, rbmDRF, bsDRF);
